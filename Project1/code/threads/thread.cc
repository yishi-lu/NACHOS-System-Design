...
//when a thread finish, the process will update some information to check that it's time to terminate or not
void Thread::myFinish(){

    kernel->interrupt->SetLevel(IntOff);

    kernel->currentProcess->threadFinished();

    cout<<"current thread finish, need one time quantum to finish current thread"<<endl;

    finish = true;

    //all threads of the process are finished, it's time to terminate
    if(kernel->currentProcess->getNumThread() == kernel->currentProcess->getFinishedThread()) kernel->currentProcess->Termination();
