...
main(int argc, char **argv)
{
    ...
    bool tqFlag = false;
    int tq = 0;//time quantum which set by user
    int position = 1;
#ifndef FILESYS_STUB
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    i++;
            ...
            tqFlag = TRUE;
            tq = (int) strtol(argv[i + 1], (char **)NULL, 10);
            i++;
    }
	else if (strcmp(argv[i], "-x") == 0) {
	    ASSERT(i + 1 < argc);
        if(tqFlag == TRUE && position == 1) position = i;
    ...
    if(tqFlag){
        //set time quantum of NACHOS
        kernel->scheduler->setTQ(tq);
    }
    ...
    if (userProgName != NULL) {
        //if there is more than one user programs need to be run
        //we create one thread to run one user program
      while(strcmp(argv[position], "-x") == 0){
        Thread *t = new Thread("forked thread");
        userProgName = argv[position + 1];
        t->Fork((VoidFunctionPtr) RunUserProg, (void *) userProgName);
        position += 2;
        if(position+1 > argc) break;
      }
    }
